...
def calc_set_damage(base, ability_level, scaling_changes):
    ...
    for i in range(8):
        if i >= 1:
            damage += (
                base * (int(scaling_changes.split("/")[ability_level - 1]) / 100)
            ) * 0.4
        else:
...
def change_special(god, ability_numbers, levels):
    ...
    if god == "Odin":
        ability_numbers.append(
            {
                "damage": str(
                    int(ability_numbers[0]["damage"])
                    + round((float(ability_numbers[1]["damage"]) * 1.15))
                ),
...
def special_description_parsing(description):
    return (
        ...
        "lane minion damage" not in description
