...
export default function SideNav() {
    ...
    const iconStyling = `w-8 h-8 ${open? '' : 'opacity-70'} hover:fill-[#3273fa]`

    return (
        <div className={`nav-border text-white px-4 py-4  ${open? 'w-16': 'w-52'}`}>
          <div>
            <GiIcons.GiHamburgerMenu className='cursor-pointer w-8 h-8 mb-7' onClick={() => setOpen(!open)}></GiIcons.GiHamburgerMenu>
          </div>
        {['Home', 'Gods', 'Tierlist', 'Contact', 'Find A Match'].map(
          (text: string, index: number) => {
            ...
            return (
                ...
                >
                  <div
                    className='cursor-pointer flex flex-row items-center mb-2'>
                      {icon}
                    <p className={`ml-2 ${open? 'hidden': ''}`}>{text}</p>
                  </div>
                </Link>
            );
          }
