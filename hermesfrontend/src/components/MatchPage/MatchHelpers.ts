...
export function GetCarryPlayer(
  damageScore: number,
  goldScore: number,
  killPart: number,
  wards: number,
  ...
  let avgDamage =
    scoringAverages['damageScore'][winStatus][role]['damageShare']['avg'];
  let stdDevDamage =
    scoringAverages['damageScore'][winStatus][role]['damageShare']['stdDev'];
  let avgDiffDamage = damageScore - avgDamage;
  let scoreDamage = avgDiffDamage % stdDevDamage;

  let avgGold =
    scoringAverages['goldScore'][winStatus][role]['goldShare']['avg'];
  let stdDevGold =
    scoringAverages['goldScore'][winStatus][role]['goldShare']['stdDev'];
  let avgDiffGold = goldScore - avgGold;
  let scoreGold = avgDiffGold % stdDevGold;

  let avgKillPart =
    scoringAverages['killPart'][winStatus][role]['killShare']['avg'];
  let stdDevKillPart =
    scoringAverages['killPart'][winStatus][role]['killShare']['stdDev'];
