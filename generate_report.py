    ...
    def upload_report(self):
        ...
        with open("projectAthena/filepath.txt", "r") as f:
            ...
            for mode in self.params["mode"]:
                ...
                if mode == "Conquest":
                    parsed_report = []
                    parsed_report = self.parse_report(self.report_data[mode].values())
                    print(parsed_report)
                    ...
                    report = pd.DataFrame(
                        ...
                        self.report_data[mode].keys(),
                        self.report_data[mode][
                            list(self.report_data[mode].keys())[0]
                        ].keys(),
                    )
